...
void* thread_action(void* pool) {
    ...
    while (1) {
        ...
        if (th_pool->m_tasks.empty()) {
            ...
            th_pool->m_busy_tasks.push_back(task);
            th_pool->m_tasks.pop();
            pthread_mutex_unlock(&th_pool->m_mutex);
...
ThreadPool::~ThreadPool() {
    ...
    for (std::size_t i = 0; i < m_busy_tasks.size(); i++) {
        pthread_cond_destroy(&m_busy_tasks[i]->m_cond);
        pthread_mutex_destroy(&m_busy_tasks[i]->m_mutex);
    }
...
void ThreadPool::submit(Task* task) {
	pthread_mutex_init(&task->m_mutex, NULL);
    pthread_mutex_lock(&task->m_mutex);
    task->m_stop = 0;
    pthread_cond_init(&task->m_cond, NULL);
    pthread_mutex_unlock(&task->m_mutex);
